...
import { ConnectionResponseDto, ConnectionStatusResponseDto } from './dto/connection-response.dto';
...
export class ConnectionsController {
  ...
  @ApiOperation({
    ...
    description: 'Accept a pending connection request (only receiver can accept)',
  })
  @ApiParam({
    name: 'id',
  ...
  async acceptConnection(@CurrentUser('id') userId: string, @Param('id') connectionId: string) {
    const connection = await this.connectionsService.acceptConnection(connectionId, userId);
  ...
  @ApiOperation({
    ...
    description: 'Reject a pending connection request (only receiver can reject)',
  })
  @ApiParam({
    name: 'id',
    description: 'Connection request ID',
  ...
  async rejectConnection(@CurrentUser('id') userId: string, @Param('id') connectionId: string) {
    const connection = await this.connectionsService.rejectConnection(connectionId, userId);
  ...
  @ApiOperation({
    ...
    description: 'Cancel a pending connection request that you sent (only sender can cancel)',
  })
  @ApiParam({
    name: 'id',
    description: 'Connection request ID',
  ...
  async cancelRequest(@CurrentUser('id') userId: string, @Param('id') connectionId: string) {
  ...
  @ApiOperation({
    ...
    description: 'Block a user, preventing all connection operations between you and them',
  })
  @ApiParam({
    name: 'userId',
  ...
  async blockUser(@CurrentUser('id') userId: string, @Param('userId') targetUserId: string) {
  ...
  @ApiOperation({
    ...
    description: 'Unblock a user that you previously blocked (deletes the connection document)',
  })
  @ApiParam({
    name: 'userId',
  ...
  async unblockUser(@CurrentUser('id') userId: string, @Param('userId') targetUserId: string) {
  ...
  @ApiOperation({
    ...
    description: 'Check the connection status between you and another user (bidirectional)',
  })
  @ApiParam({
    name: 'userId',
  ...
  async getConnectionStatus(
    ...
    const status = await this.connectionsService.getConnectionStatus(userId, targetUserId);
